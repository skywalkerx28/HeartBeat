...
export function DivisionWatch({ standings, isLoading }: DivisionWatchProps) {
  ...
  return (
    <div className="relative group overflow-hidden rounded-lg">
      ...
      <div
        className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent backdrop-blur-md
                   border transition-all duration-300 rounded-lg
                   border-red-600/20 shadow-md shadow-red-600/10
                   group-hover:border-red-600/60 group-hover:shadow-xl group-hover:shadow-red-600/30" />
      {/* Subtle scanlines */}
      <div className="pointer-events-none absolute inset-0 opacity-10">
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-white/10 to-transparent animate-pulse" />
      </div>
      {/* Corner accents */}
      <div className="pointer-events-none absolute top-2 left-2 w-2 h-2 border-t border-l border-white/20 rounded-sm" />
      <div className="pointer-events-none absolute top-2 right-2 w-2 h-2 border-t border-r border-white/20 rounded-sm" />
      <div className="pointer-events-none absolute bottom-2 left-2 w-2 h-2 border-b border-l border-white/20 rounded-sm" />
      <div className="pointer-events-none absolute bottom-2 right-2 w-2 h-2 border-b border-r border-white/20 rounded-sm" />
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center space-x-3">
                  <div className={`
                    flex items-center justify-center w-8 h-8 rounded bg-white/5 border
                    ${isMTL ? 'border-red-600/50' : 'border-white/20'} overflow-hidden
                  `}>
                    <img
                      ...
                      className={
                        ...
                        (teamAbbrev === 'MTL' 
                          ? '' 
                          : teamAbbrev === 'TBL' 
                            ? 'grayscale opacity-280'  // Further lighten for Tampa Bay
                  ...
                  <div>
                    <div className={`text-sm font-military-display ${isMTL ? 'text-white' : 'text-gray-300'}`}>
                      {teamAbbrev}
                    </div>
                ...
                <div className="flex items-center space-x-4">
                  <div className="text-right">
                    <div className="text-xs font-military-display text-gray-500">PTS</div>
                    <div className={`text-sm font-military-display tracking-wide ${isMTL ? 'text-white' : 'text-gray-300'}`}>
                      {points}
                    </div>
                  </div>
                  
                  <div className="text-right">
                    <div className="text-xs font-military-display text-gray-500">DIFF</div>
